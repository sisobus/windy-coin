...
## 0. 빠른 시작 (recommended) — `scripts/mine.sh`
...
Looks mintable. Re-run without --dry-run to actually claim 10.0 WNDY.
```

eligibility 위반 시 (visited_cells out of range, score < 10) 친절하게 경고.

### 0.2 실 채굴 — argument 없이 (5~10분)

dry-run으로 만족스러우면:

```bash
./scripts/mine.sh circuit/programs/my_program.wnd
```

스크립트가 알아서:
1. Docker 실행 여부 + 키스토어 존재 여부 확인
2. recipient 주소를 `deployer-mainnet` 키스토어에서 자동 추출
3. `cargo run -p host -- ...` 호출해 Groth16 proof 생성 (5~10분, 비번 불필요)
4. 출력에서 `seal` + `journal` 파싱
5. mainnet의 `computeScore`로 한 번 더 점수 확인
6. `cast send` 으로 mainnet mint 전송 (이 단계에서만 키스토어 비번 prompt)
7. 결과 + 잔액 + totalSupply 출력
...
수동 흐름이 궁금하면 §5 이하 (스크립트가 하는 일을 풀어쓴 것).
