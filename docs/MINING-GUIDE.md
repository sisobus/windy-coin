...
> windy-lang 자체의 문법은 [sisobus/windy-lang](https://github.com/sisobus/windy-lang)
...
## 0. 빠른 시작 (recommended) — `windy mine` 또는 `windy-mine`

본인 `.wnd` 파일 경로만 넘기면 proof 생성부터 mint 전송까지 한 번에 처리합니다.

### 0.0 설치 (한 번만)

```bash
cargo install windy-lang  # ≥ v2.3.0, windy CLI + plugin lookup
cargo install windy-mine  # 채굴 바이너리
```

설치 후 세 가지 호출 방법 모두 동작:

```bash
windy mine path/to/program.wnd       # windy-lang의 plugin 경로 (추천)
windy-mine path/to/program.wnd       # windy-coin의 바이너리 직접
./scripts/mine.sh path/to/program.wnd  # bash 스크립트 (repo clone 시)
```

> ⚠️ **v2.2.x 이하의 windy-lang은 `windy mine` 을 모릅니다.** `windy --help`
> 출력에 `mine` 항목이 있는지 확인. 없으면 `cargo install windy-lang` 으로
> 업그레이드.

### 0.1 채점만 — `--dry-run` (수십 초)

본인 코드가 몇 점인지, 어떤 tier인지 먼저 확인:

```bash
windy mine --dry-run circuit/programs/my_program.wnd
```

- **Docker 불필요** — Groth16 wrap 스킵, zkVM 실행만 (~수십 초)
- **트랜잭션 없음** — mainnet에 아무것도 안 보냄, 가스 0
- **mainnet의 `computeScore` 호출**해서 정확한 점수 + tier 출력
- 코드 짜는 동안 빠른 iteration 도구

출력 예 (fib_gold.wnd 기준):
```
...
eligibility 위반 시 (visited_cells out of range, score < 10) 친절하게 경고.

### 0.2 실 채굴 — `--dry-run` 빼고 (5~10분)

dry-run으로 만족스러우면:

```bash
windy mine circuit/programs/my_program.wnd
```

CLI가 알아서:
1. Docker 실행 여부 확인
2. recipient 주소를 `deployer-mainnet` 키스토어에서 자동 추출
3. Risc Zero Groth16 proof 생성 (5~10분, 비번 불필요)
4. mainnet의 `computeScore`로 점수 사전 검증 (eligibility 안 맞으면 tx 안 보냄)
5. `cast send` 으로 mainnet mint 전송 (이 단계에서만 키스토어 비번 prompt)
6. 결과 + 잔액 + totalSupply 출력

플래그로 오버라이드 (필요 시):
- `--account other-keystore` — 다른 키스토어 사용
- `--recipient 0x...` — recipient를 키스토어와 다른 주소로
- `--nonce 0x...` — 결정적 nonce 사용 (재현성 필요할 때)
- `--rpc ...` — 다른 Base RPC 사용

오류 발생 시 revert reason까지 친절하게 안내 (ProgramAlreadyConsumed / ScoreBelowFloor / VisitedCellsOutOfRange 등).

### 0.3 (이전) `scripts/mine.sh`

bash 스크립트 버전도 그대로 유지됩니다 (`cargo install` 안 했거나 디버깅 용도). 동일 UX:

```bash
./scripts/mine.sh --dry-run circuit/programs/my_program.wnd
```

수동 흐름이 궁금하면 §5 이하 (CLI가 하는 일을 풀어쓴 것).
...
자세한 건 [sisobus/windy-lang README](https://github.com/sisobus/windy-lang)와
