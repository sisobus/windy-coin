...
```bash
# windy-coin의 채굴 CLI — windy-mine 바이너리 설치
cd ~/coding/sisobus-workspace/windy-coin
cargo install --path cli

# windy-lang CLI (≥ v2.3.0, plugin 지원 포함)
# crates.io에 v2.3.0 publish됐으면:
cargo install windy-lang
# 아니면 로컬 clone에서:
cd ~/coding/sisobus-workspace/windy
cargo install --path .
```

> ⚠️ **v2.2.x 이하의 windy-lang은 `windy mine` 을 모릅니다.** 이미 깔린 게
> 2.2.x 이하라면 위 명령으로 덮어쓰기. 확인: `windy --help` 출력에 `mine`
> 항목이 있어야 합니다.

설치 후 두 가지 호출 방법 모두 동작:

```bash
windy mine path/to/program.wnd       # windy-lang의 plugin 경로
